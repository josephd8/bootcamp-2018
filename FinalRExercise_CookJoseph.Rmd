---
title: "FinalRExercise_CookJoseph"
author: "Joseph Cook"
...
```{r}
schools <- read.csv(here::here("data/nys_schools.csv"), header = TRUE, 
                    check.names = FALSE, stringsAsFactors = FALSE)
counties <- read.csv(here::here("data/nys_acs.csv"), header = TRUE, 
                    check.names = FALSE, stringsAsFactors = FALSE)
```
...
apply(schools,2,function(x){
  
  x[which(x == -99)] <- NA
  
})

apply(counties,2,function(x){
  
  x[which(x == -99)] <- NA
  
})

schools[complete.cases(schools),]
counties[complete.cases(counties),]

### Create Poverty Groups Variable
# I took the 33% and 66% quantiles of county_per_poverty to find values that split the counties into thirds (high, medium, low)

qs <- quantile(counties$county_per_poverty,probs = c(.33,.66))

counties <- counties %>%
  mutate(Group = cut(county_per_poverty, 
                     breaks = c(0,qs[1], qs[2],100),
...
data <- schools %>%
  merge(counties, by.x = c("county_name","year"), by.y = c("county_name","year"))
...
data %>%
  ...
  summarize(Enrollment = sum(total_enroll),
            FreeOrReducedLunch = mean(per_reduced_lunch + per_free_lunch),
...
qs <- quantile(counties$county_per_poverty,c(.05,.95))

data %>%
  filter(county_per_poverty <= qs[1] | county_per_poverty >= qs[2]) %>%
  group_by(county_name) %>%
  summarize(FreeOrReducedLunch = mean(per_free_lunch + per_reduced_lunch), MeanMath = mean(scaled_math), MeanReading = mean(scaled_ela))
...
data %>% 
  ...
  summarize(FreeReducedLunch = mean(per_free_lunch + per_reduced_lunch),
            TestPerformance = mean(scaled_ela + scaled_math)) %>%
  ggplot() +
  geom_point(aes(x = FreeReducedLunch, y = TestPerformance))
...
data %>%
  group_by(Group) %>%
  select(Group, scaled_ela, scaled_math) %>%
  melt(id.vars = "Group") %>%
  ggplot() +
  geom_boxplot(aes(y = value, x = Group)) + 
